...
struct WeatherView: View {
    ...
    private func detailCard(icon: String, value: String, label: String, tint: Color) -> some View {
        DetailCard(icon: icon, value: value, label: label, tint: tint)
    }
...
/// Matches HoverButton's hover language: a background fill that brightens
/// on hover, no scale/brightness transform on the content itself.
private struct DetailCard: View {
    ...
    var body: some View {
        ...
        .background(RoundedRectangle(cornerRadius: 10).fill(tint.opacity(isHovering ? 0.25 : 0.15)))
        .onHover { hovering in
