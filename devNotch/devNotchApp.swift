...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("devnotch", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings") {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var viewModels: [String: DevViewModel] = [:] // UUID -> DevViewModel
    var window: NSWindow?
    let vm: DevViewModel = .init()
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? DevNotchSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? DevNotchSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createDevNotchWindow(for screen: NSScreen, with viewModel: DevViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = DevNotchSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createDevNotchWindow(
    ...
    func playWelcomeSound() {
        ...
        audioPlayer.play(fileName: "dev", fileExtension: "m4a")
    }
    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = DevViewModel(screenUUID: uuid)
                    let window = createDevNotchWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createDevNotchWindow(for: selectedScreen, with: vm)
            }
