...
class DevViewModel: NSObject, ObservableObject {
    @ObservedObject var coordinator = DevViewCoordinator.shared
    @ObservedObject var detector = FullscreenMediaDetector.shared

    let animationLibrary: DevAnimations = .init()
    ...
    init(screenUUID: String? = nil) {
        ...
        aerospaceManager = AerospaceManager(nsScreenIndex: NSScreen.index(forUUID: screenUUID) ?? 1)
